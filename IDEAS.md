...
## IDEA-20260424-01: Agent 实时通讯 — WebSocket 消息总线
- **Date**: 2026-04-24
- **Category**: infrastructure
- **Tags**: agent-communication, websocket, rustclaw, openclaw, ipc
- **Triggered by**: potato 想让 RustClaw 和 OpenClaw 实时互发消息

### The Idea
一个极简 WebSocket 消息总线，让多个 agent（RustClaw / OpenClaw / 以后的）可以实时互发消息。不是社交平台，不是 RPC，就是一个 bus。

### 架构
```
┌──────────┐      ┌─────────────────┐      ┌──────────┐
│ RustClaw │◄────►│  agent-bus (WS) │◄────►│ OpenClaw │
└──────────┘      │  - registry     │      └──────────┘
                  │  - routing      │
                  └─────────────────┘
```
...
### 协议（JSON over WS）
- `{"type":"hello","agent":"rustclaw"}` — 注册
- `{"type":"msg","to":"openclaw","from":"rustclaw","content":"..."}` — 定向
- `{"type":"broadcast","from":"rustclaw","content":"..."}` — 广播
- Server 推送同样结构，加 `ts` 字段

### 组件
1. **agent-bus server** (~100 行 Rust)
   - `tokio-tungstenite` + `HashMap<agent_name, WsSender>`
   - 监听 `127.0.0.1:9876`（本机 only）
   - 收 msg → 查表转发，不在线丢弃
   - 独立 binary，launchd 守护

2. **RustClaw AgentBusChannel** (~150 行)
   - 跟 TelegramChannel 同级
   - 连 `ws://localhost:9876`，发 hello
   - 收到消息 → 包成 `MessageContext` → 现有 LLM 路径
   - LLM 回复 → 发 `msg` 回去

3. **OpenClaw 对应 channel**
   - 同样逻辑，复用协议

### MVP 决策
- Server 位置：独立 binary + launchd 守护
- 离线消息：丢弃（不做队列）
- 权限：本机 only + shared secret
- 历史：不存，需要时 agent 自己 engram

### 工作量
~2 天（server 半天 + 两个 channel 各半天 + 联调半天）

### 为什么值得做
- 现在 agent 间只能共享文件系统 + engram，异步、被动、要轮询
- 有了 bus → 实时、主动、事件驱动
- RustClaw 和 OpenClaw 可以真正协作（"帮我跑个测试"、"我做完了，你继续"）
- 未来加云端 agent、第三方 agent 都走同一个接口

### Why Not SaltyHall?
之前想过复用 SaltyHall 做 agent 通信层。太重：
- SaltyHall 是 public social platform，agent 内部通信走那儿 = leak + 依赖外部服务
- 本地 WS bus 更合适 — 私有、零延迟、零外部依赖
- SaltyHall 是"对外的大厅"，agent-bus 是"内部的对讲机"，两件事

### Next Steps
- [ ] 起 `crates/agent-bus` crate（server）
- [ ] RustClaw 加 `AgentBusChannel`
- [ ] OpenClaw 加对应 channel
- [ ] 联调：RustClaw 发 "ping" → OpenClaw 收到回 "pong"
