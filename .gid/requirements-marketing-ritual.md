...
管道定义：**Topic Selection → 内容生产 → 分发 → 互动管理 → 数据回收 → 自我优化**

> **术语说明：** Content Ritual 的第一阶段称为 "Topic Selection"（选题/素材输入），区别于 social-intake skill（外部内容提取和存储）。文档中 "intake" 如未加限定，指 social-intake skill。
...
### 0. 启动与引导（Setup & Bootstrap）

系统首次运行和冷启动阶段的特殊处理。

- **GOAL-0.1** [P0]: 冷启动阶段（前 30 天）的完整流程：(1) Style Profile 从 Telegram + 种子样本构建（GOAL-2.6），(2) analytics 基线逐步积累——发布内容不足 10 条时不进行对比分析，(3) GOAL-5.2 周报在第 2 周后开始生成，(4) Module 8 自我优化在 30 天基线建立前禁用。系统在引导期明确告知 potato："处于引导阶段，还需 N 条内容才能启动完整分析" *(ref: FINDING-9 冷启动场景)*
- **GOAL-0.2** [P0]: 前置条件检查——系统启动前验证：xinfluencer 已安装且可用、API keys 已配置（Twitter OAuth 2.0）、engram 可连接、Telegram bot token 有效。任一前置条件不满足时给出明确错误信息和修复指引 *(ref: FINDING-12 Module 0)*

### 1. 内容生产（Content Production）

potato 提供一个 topic/素材/intake 条目，系统自动生成适合目标平台的内容草稿。

- **GOAL-1.1** [P0]: 给定一个 topic 或 intake 条目引用（如 `intake/twitter/xxx.md`），能生成一篇内容草稿，包含：标题/hook、正文、hashtags/tags。错误处理见 GOAL-7.6（draft 生成失败重试）*(ref: Content Ritual 讨论 04/03)*
...
- **GOAL-1.3a** [P1]: 平台特定约束（字符限制、格式规则、媒体要求）维护在配置文件 `config/platforms.yaml` 中，不硬编码。内容生成步骤从该配置读取平台约束。配置包含：max_length、supports_markdown、supports_media、supports_threads、rate_limit *(ref: FINDING-11 平台约束可配置化)*
...
- **GOAL-2.1** [P0]: 从 potato 的多源历史内容（推文、Telegram 聊天、代码注释/commit messages）中提取写作风格特征：常用词汇、句式偏好、语气、幽默方式、中英混用习惯、平台差异。Style Profile 验证标准：(1) potato 审核并批准生成的 profile（人工验收测试），(2) LLM-as-judge 对 5 篇 potato 真实文本与 profile 的匹配度评分 > 0.8（校准测试），(3) LLM-as-judge 对 5 篇通用 LLM 文本与 profile 的匹配度评分 < 0.4（区分度测试）。Profile 生成仅在通过全部 3 项验证后视为完成 *(ref: "去 AI 感" 04/03)*
...
- **GOAL-3.1** [P0]: 支持 Twitter/X 发布（推文、thread、带图推文）——通过 Twitter API v2，认证方式为 OAuth 2.0 PKCE。实现为 xinfluencer 的新 `publish` 模块或独立发布服务。**Blocked-by:** xinfluencer publish 模块（待新增，见 Dependencies） *(ref: xinfluencer publish 模块，见 Dependencies)*
- **GOAL-3.2** [P2]: 阶段二新增至少 2 个平台的分发——Hacker News、Reddit（不含阶段一已有的 Twitter/X） *(ref: potato 的核心受众平台，阶段二实施)*
- **GOAL-3.3** [P1]: 同一内容自动适配不同平台格式——Phase 1: 内容生成遵循 `config/platforms.yaml`（GOAL-1.3a）中的平台特定约束。Phase 2: 同一内容自动适配不同平台格式（Reddit 版详细有深度，HN 版技术聚焦） *(ref: 多平台策略)*
...
- **GOAL-5.1** [P1]: 对每条发布内容追踪效果数据：impressions、engagement rate、reply depth、follower conversion、link CTR。数据来源：xinfluencer monitor 模块（使用 Twitter API v2 engagement endpoints）。采集频率：发布后 1h、6h、24h、7d（每条内容 4 个数据点）。Twitter API 不可用时指数退避重试；超过 24h 不可用则标记 metrics 为 `incomplete`，以已采集数据继续后续分析 *(ref: 数据驱动优化, FINDING-4 数据源与采集频率明确化)*
...
- **GOAL-6.1** [P0]: 周期性扫描 potato 关注的账号、话题、关键词，发现高价值内容（通过 xinfluencer discover 模块）。调用方式：CLI (`xinfluencer discover --topics <topics> --output json`) 或 library API (`xinfluencer::discover::find_content(topics, config) -> Vec<DiscoveredContent>`)。marketing-ritual 依赖的输出格式：`{url, author, score, topic, timestamp}`。marketing-ritual 锁定 xinfluencer 输出 schema 版本 *(ref: proactive intake 需求 04/03, FINDING-10 接口规范)*
...
- **GOAL-7.0** [P1]: potato 的典型工作流：收到 Telegram 通知 → 审核内容草稿（approve/edit/reject） → 审核互动建议 → 查看周报。potato 的时间投入目标：每天内容管理 < 15 分钟。系统设计以此用户体验为导向 *(ref: FINDING-14 用户视角工作流)*

- **GOAL-7.1** [P0]: 实现 Content Ritual 状态机，包含 6 个 phase：topic-selection → draft → review → schedule → publish → analyze。每个 phase 有明确的输入/输出和转换条件（见下方 GOAL-7.1a-d）。Phase 转换需满足对应条件，不可跳跃（除 GOAL-7.5 配置化跳过外） *(ref: Content Ritual 同构讨论 04/03)*
- **GOAL-7.1a** [P0]: Phase 转换：topic-selection → draft。触发条件：potato 提供 topic 或 intake 引用。输入：topic 字符串或 intake 文件路径。输出：draft 内容 + metadata *(ref: FINDING-6 状态机拆分)*
- **GOAL-7.1b** [P0]: Phase 转换：draft → review。触发条件：draft 生成成功。输入：draft 内容。输出：potato 的决策（approve / edit / reject / schedule） *(ref: FINDING-6 状态机拆分)*
- **GOAL-7.1c** [P0]: Phase 转换：review → schedule/publish。触发条件：potato approve。输入：已审核内容。输出：设定发布时间（schedule）或立即发布（publish） *(ref: FINDING-6 状态机拆分)*
- **GOAL-7.1d** [P0]: Phase 转换：publish → analyze。触发条件：发布成功。输入：已发布内容 ID。输出：启动 metrics 采集流程（GOAL-5.1） *(ref: FINDING-6 状态机拆分)*
...
- **GOAL-7.6** [P0]: Draft 生成失败时（LLM 错误、超时、rate limit、context 过长、Style Profile 不可用），自动重试最多 3 次（指数退避），持续失败则通知 potato 并将任务标记为 blocked，附带 partial output（如有）供后续重试。所有 LLM 调用错误统一走此处理流程 *(ref: 可靠性 — draft phase 错误处理, FINDING-3 错误处理强化)*
- **GOAL-7.7** [P1]: 发布失败时自动重试（指数退避，最多 3 次），持续失败则通知 potato 并保存草稿到队列。API 限流时自动延后发布时间 *(ref: 可靠性)*
- **GOAL-7.8** [P0]: 内容持久化模型——内容项以结构化文件存储在 `content/{status}/{YYYY-MM-DD}-{slug}.md`，包含 YAML frontmatter（content_id、status、platform、created_at、published_at、metrics）。Status 目录：drafts/、approved/、published/、archived/。内容 metadata 同时作为 GID graph nodes 追踪，支持依赖和影响分析 *(ref: FINDING-7 内容持久化模型)*

### 8. 自我优化（Self-Improvement）

这套管道跑得越多应该越好，不是静态的。自我优化能力依赖 **gepa-core**（独立 crate，将发布到 crates.io）作为外部依赖。

> **⚠️ 注意：** Module 8 大部分被 gepa-core (crate) + self-improvement system 阻塞。GOAL-8.1a 的 heuristic 分析是唯一可在 Phase 1 独立实现的部分，其余 GOALs 实际为 P2。

- **GOAL-8.1a** [P1]: 基于 execution trace 自动识别管道中的薄弱环节（哪个 phase 耗时最长、哪个 phase 被 potato reject 最多）。用 heuristic 实现（统计分析），不依赖 gepa-core *(ref: Skill 优化系统)*
- **GOAL-8.1b** [P2]: GOAL-8.1a 的 gepa-core 集成版本——将 heuristic 分析升级为 gepa-core 驱动的智能优化分析 *(ref: Skill 优化系统, gepa-core 集成)*
- **GOAL-8.2** [P2]: 对表现差的 skill/prompt 生成改进建议（非自动修改），potato 审批后应用 *(ref: GEPA 方法论, Hermes Agent self-evolution)*
...
- **GUARD-1** [hard]: 所有自动生产的内容，在发布前必须经过 potato 确认（至少看一眼）。系统可以草拟、可以排队、但不能自动发布。唯一例外：potato 通过 Telegram 对特定内容类型明确开启 auto-publish 标记（如 quote tweets），该标记需每 30 天重新确认，过期自动回退为需审批。Auto-publish 内容必须额外通过 GUARD-2 验证：发布前执行轻量级 fact-check 步骤（LLM self-review："此内容是否包含我无法验证的声明？"），如 self-review 标记任何不确定声明，该内容自动转入人工审核流程而非 auto-publish *(ref: SOUL.md "ask first for external actions", FINDING-5 auto-publish 与 GUARD-2 一致性)*
...
1. **Style Profile 质量** — 风格蒸馏是主观评估，LLM 提取的风格特征可能不准确。在围绕 Style Profile 构建完整管道前，先让 potato 验证 profile 质量（GOAL-2.1 验证标准）
2. **Twitter API 发布** — 外部依赖，API 政策可能变化。先用测试账号验证发布流程，再接入主账号
3. **反 AI 检测（GOAL-2.5）** — LLM 检测 LLM 输出是开放研究问题，准确率存疑。作为 P2 增强功能，不作为核心流程依赖
...
- **xinfluencer**（Rust crate）— crawler（推文历史抓取）、discover（高价值内容发现，输出格式：`{url, author, score, topic, timestamp}`）、engage（互动）、monitor（效果监控，使用 Twitter API v2 engagement endpoints）、autopilot（自动化流程）、**publish（待新增）**——用于发布推文/thread，当前不存在，需作为新模块开发。marketing-ritual 锁定 xinfluencer 输出 schema 版本
...
2. **Content Ritual 完整状态机** — topic-selection → draft → review → schedule → publish → analyze *(依赖 Style Profile 定义 draft 阶段的风格输入)*
...
**53 GOALs** (21 P0 / 20 P1 / 12 P2) + **7 GUARDs** (3 hard / 4 soft)
