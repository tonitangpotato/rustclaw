...
- **GOAL-7.10** [P2]: 系统级成功指标：(1) 至少 1 个维度的 primary metric 在 30 天内有统计显著的提升（p < 0.05 on paired test, or > 5% absolute improvement with > 20 data points），(2) potato 的 approve rate > 50%（优化结果不总是被 reject），(3) 自动回滚率 < 30%（部署的优化大多数站得住）。
