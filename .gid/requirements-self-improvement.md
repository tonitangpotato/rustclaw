...
- **GOAL-1.1** [P0]: 追踪每个 skill 的使用事件——触发次数、触发准确率（误触发率 = 不应触发但触发了 / 总触发数；漏触发率 = 应触发但没触发 / 总应触发数）、输出质量评分（来自用户反馈：纠正 = 0分、无反馈 = 0.5分、正面反馈 = 1分）。漏触发检测数据来源：(a) 用户手动使用了 skill 的功能（如发送特定命令或关键词）而自动触发未命中，(b) 用户在对话中纠正说 "你应该用 XX skill"。数据持久化到 `.gid/skill-metrics/{skill-name}.jsonl` *(ref: IDEA-20260403-01)*
...
- **GOAL-2.1** [P0]: 识别 system prompt 中的可优化 sections——AGENTS.md 中的 workflow rules、communication style 指令、behavior rules 是可优化的；SOUL.md 的 core identity 和 safety rules 不可优化（GUARD-1） *(ref: system prompt modular architecture)*
...
- **GOAL-3.1** [P0]: 分析 execution-log.jsonl 中的执行轨迹，自动分类错误类型：工具选择错误（调用了错误的工具）、参数错误（工具正确但参数不对）、超时（工具调用超过 timeout）、幻觉（输出包含不存在的事实/文件/路径）、格式错误（输出格式不符合 channel 要求）、逻辑错误（步骤顺序错误或遗漏关键步骤）。分类方法：LLM-as-judge 对每条 trace 标注类别 + 严重程度（low/medium/high） *(ref: execution-log.jsonl format)*
...
- **GOAL-4.1** [P1]: 追踪记忆召回精度——每次 engram_recall 调用后，判断召回的记忆是否被实际使用（出现在后续回复中）或被忽略，计算召回精度（used / retrieved）。判断方法：对比 recall 结果与最终 response 的语义重叠度
...
- **GOAL-6.1** [P0]: 从执行历史自动生成 synthetic test cases——提取（用户消息, agent 响应, 用户反馈）三元组，标注正面/负面。正面 = 无用户纠正且任务完成；负面 = 用户纠正或明确否定。每个 test case 包含：input（用户消息 + context snapshot）、expected_behavior（从正面案例提取）、tags（关联的 skill/prompt section） *(ref: Hermes Agent self-evolution)*
...
- **GOAL-7.3** [P0]: 自动回滚——如果已部署的优化版本在后续 M 次使用（可配置，默认 10）中表现低于基线版本（定义：该维度最近一次被 potato approve 的版本；首次优化时基线为优化前的原始版本），自动回滚到基线版本并通知 potato
...
- **GUARD-1** [hard]: SOUL.md 中的 core identity（"Core Truths"、"Boundaries"、"Honesty Rules"）和安全规则永远不被优化系统修改。这些 sections 在 SystemPromptAdapter 中被标记为 frozen，GEPA 的 mutation 操作跳过这些参数。违反 = agent 安全边界被破坏。
